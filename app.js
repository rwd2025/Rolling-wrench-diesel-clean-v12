...
async function rw8Run(text){rw8Ensure();text=String(text||"").trim();if(!text)return;const intent=rw8Intent(text);if(intent==="clear"){state.brainChats=[];saveState();renderRW8Brain();return;}if(intent==="open_quote"){setRoute("quotes");return;}if(intent==="parts_action"){setRoute("parts");return;}if(intent==="invoice_action"){setRoute("invoices");return;}rw8ExtractMemory(text);rw8Push("user",text);if(intent==="quote"){const q=rw8Quote(text);state.quotes.unshift(q);state.rw8.lastCard={type:"quote",quote:q};saveState();rw8Push("ai",`I found a ${q.title.toLowerCase()}.\n\nLabor: ${q.hours} hrs\nEstimated Total: ${money(q.total)}\n\nI saved a quote draft.`);rw8Push("ai","quote card","card",state.rw8.lastCard);}else if(intent==="diagnostic"){const d=rw8Diagnostic(text);state.notes.unshift({type:"AI Diagnostic",note:d,date:new Date().toLocaleString()});state.rw8.lastCard={type:"diagnostic",text:d};saveState();rw8Push("ai",d,"card",state.rw8.lastCard);}else if(intent==="invoice"){const ctx=rw8Context();state.invoices.unshift({customer:ctx.customer,truck:ctx.truck,work:text,total:0,status:"Draft",ai:true,date:new Date().toLocaleString()});saveState();rw8Push("ai","I built an invoice draft and saved it to Invoices.");}else if(intent==="memory"){const ctx=rw8Context();if(typeof ensureV691==="function")ensureV691();state.repairMemory.unshift({id:"MEM-RW8-"+Date.now(),title:text.slice(0,55),complaint:text,cause:"",correction:"",customer:ctx.customer,truck:ctx.truck,engine:ctx.engine,keywords:text,status:"Saved",date:new Date().toLocaleString(),ai:true});saveState();rw8Push("ai","Saved to Repair Memory.");}else if(intent==="vision"){rw8Push("ai","Open OCR/Vision and attach the photo. Once the real vision backend is connected, I’ll read the image like ChatGPT/Gemini.");}else{rw8Push("ai","RW AI cleanup router is active. Ask a question, build a quote/invoice, decode VIN, or search parts.");}renderRW8Brain();}
...
/* ===== RWD AI CLEANUP CORE — SINGLE ROUTER =====
   Old V8.6/V8.7 AI fallback paths are disabled from controlling chat.
   AI now routes through one path:
   Parts -> oracle-parts-search
   VIN -> truck-search with local fallback
   Quote -> build-quote with finance fallback
   Invoice -> build-invoice with finance fallback
   General -> diesel-doc-ai with Render fallback
*/
window.RWD_AI_CLEANUP_VERSION = "AI Cleanup Core";
const RWD_SUPABASE_URL_CLEAN = "https://uxpkqwcmvtqvubibbrek.supabase.co";
const RWD_SUPABASE_FUNCTIONS_CLEAN = RWD_SUPABASE_URL_CLEAN + "/functions/v1";
const RWD_SUPABASE_ANON_CLEAN = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InV4cGtxd2NtdnRxdnViaWJicmVrIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzcyMzk4NjQsImV4cCI6MjA5MjgxNTg2NH0.afiaSFqkRFEXW5nPQVRXKZcpKkS6iF3T_hTQC2P15HQ";
const RWD_RENDER_BACKEND_CLEAN = "https://rolling-wrench-ai-backend.onrender.com";

(function rwdCleanupBoot(){
  try {
    localStorage.setItem("RW_SUPABASE_ANON_KEY", RWD_SUPABASE_ANON_CLEAN);
    state.settings = state.settings || {};
    state.settings.supabaseAnonKey = RWD_SUPABASE_ANON_CLEAN;
    state.quotes = Array.isArray(state.quotes) ? state.quotes : [];
    state.invoices = Array.isArray(state.invoices) ? state.invoices : [];
...
function rwdCleanHeaders(){
  return {
    "Content-Type":"application/json",
    "apikey":RWD_SUPABASE_ANON_CLEAN,
    "Authorization":"Bearer " + RWD_SUPABASE_ANON_CLEAN
  };
}
async function rwdCleanFn(name, payload){
  const res = await fetch(RWD_SUPABASE_FUNCTIONS_CLEAN + "/" + name, {
    method:"POST",
    headers:rwdCleanHeaders(),
    body:JSON.stringify(payload || {})
  });
  const txt = await res.text();
  let data;
  try { data = JSON.parse(txt); } catch(e) { data = {text:txt}; }
  if(!res.ok) throw new Error(data.error || data.message || data.details || txt || name + " failed");
  return data;
}
async function rwdCleanRender(path, payload){
  const res = await fetch(RWD_RENDER_BACKEND_CLEAN + path, {
    method:"POST",
    headers:{"Content-Type":"application/json"},
    body:JSON.stringify(payload || {})
  });
  ...
  let data;
  try { data = JSON.parse(txt); } catch(e) { data = {text:txt}; }
  if(!res.ok) throw new Error(data.error || data.message || txt || path + " failed");
  return data;
}
function rwdCleanAnswer(data){
  if(!data) return "";
  if(typeof data === "string") return data;
  return data.answer || data.response || data.message || data.text || data.result || data.content || JSON.stringify(data,null,2);
}
function rwdCleanVIN(text){
  const m = String(text||"").toUpperCase().match(/\b[A-HJ-NPR-Z0-9]{17}\b/);
  return m ? m[0] : "";
}
function rwdCleanMake(vin){
  vin=String(vin||"").toUpperCase();
  if(vin.startsWith("1NK")) return "Kenworth";
  if(vin.startsWith("1XP")) return "Peterbilt";
  if(vin.startsWith("3AK") || vin.startsWith("1FU")) return "Freightliner";
  if(vin.startsWith("2HS") || vin.startsWith("3HS") || vin.startsWith("1HT")) return "International";
  if(vin.startsWith("4V4")) return "Volvo";
  if(vin.startsWith("1GB") || vin.startsWith("1GC")) return "Chevrolet/GMC";
  return "Truck";
}
function rwdCleanYear(vin){
  const map={A:2010,B:2011,C:2012,D:2013,E:2014,F:2015,G:2016,H:2017,J:2018,K:2019,L:2020,M:2021,N:2022,P:2023,R:2024,S:2025,T:2026,V:2027,W:2028,X:2029,Y:2030};
  return map[String(vin||"")[9]] || "";
}
function rwdCleanLocalVin(vin){
  vin = rwdCleanVIN(vin) || String(vin||"").toUpperCase();
  return "# VIN Decode\n- VIN: " + vin + "\n- Make: " + rwdCleanMake(vin) + "\n- Year: " + (rwdCleanYear(vin)||"Unknown") + "\n- Type: Truck\n- Confidence: Medium\n\nVerify full OEM/dealer build sheet before ordering parts.";
}
function rwdCleanParse(text){
  const s=String(text||"").replace(/\r/g," ").replace(/\n/g," ");
  const f={customer:"",truck:"",vin:"",job:"",labor:null,rate:null,parts:null,supplies:null,service:null};
  let m;
  ...
  f.vin=rwdCleanVIN(s); if(!f.truck && f.vin) f.truck=f.vin;
  ...
  if(/\b(in[- ]house|service call\s*[:\-]?\s*(off|no|none|0|\$0))\b/i.test(s)) f.service=0;
    ...
    m=s.match(/service call\s*[:\-]?\s*\$?\s*([0-9,]+(?:\.[0-9]+)?)/i);
    if(m) f.service=Number(m[1].replace(/,/g,""));
    else if(/service call\s*[:\-]?\s*(on|yes)/i.test(s)) f.service=Number(state?.settings?.serviceCall||250);
  }
  m=s.match(/(?:job|work|description|repair)\s*[:\-]\s*(.+)$/i); if(m) f.job=m[1].trim();
  if(!f.job && /clutch/i.test(s)) f.job="Clutch replacement";
  return f;
}
function rwdCleanFinanceText(prompt, type){
  const f=rwdCleanParse(prompt);
  ...
  const supplies=Number(f.supplies || 0);
  const total=hours*rate + service + parts + supplies;
  let out = "# " + (type==="invoice" ? "Invoice Preview" : "Quote Preview") + "\n\n";
  if(f.customer) out += "## Customer\n" + f.customer + "\n\n";
  if(f.vin) out += rwdCleanLocalVin(f.vin).replace("# VIN Decode","## VIN Decode") + "\n\n";
  out += "## Work\n" + (f.job || "Repair estimate") + "\n\n";
  out += "## Charges\n- Labor: " + hours + " hrs @ $" + rate.toFixed(2) + " = $" + (hours*rate).toFixed(2) + "\n";
  out += "- Parts: $" + parts.toFixed(2) + "\n- Supplies: $" + supplies.toFixed(2) + "\n- Service Call: $" + service.toFixed(2) + "\n\n";
  out += "## Estimated Total\n$" + total.toFixed(2);
  return out;
}
function rwdCleanLooksParts(q){
  const s=String(q||"").toLowerCase();
  return /\b\d{5,}\b/.test(s) || s.includes("cross reference") || s.includes("part number") || s.includes("fleetguard") || s.includes("baldwin") || s.includes("wix") || s.includes("donaldson") || s.includes("napa") || s.includes("water pump");
}
function rwdCleanPartQuery(q){
...
function rwdCleanFormatParts(data, q){
  const part=rwdCleanPartQuery(q);
  const d=data||{}, look=d.lookup||d.data||d.result||d;
  let out="# Parts Master Result — " + part + "\n\n";
  ...
  if(Array.isArray(crosses)&&crosses.length){
    out+="\n## Cross References\n";
    crosses.forEach(x=>{ out += typeof x==="string" ? "- "+x+"\n" : "- "+(x.brand||x.make||"Cross")+": "+(x.part_number||x.partNumber||x.number||x.value||"")+"\n"; });
  } else {
    out+="\n## Result\n- Search completed, but no verified cross-reference list was returned.\n";
  }
  out+="\n## Verify Before Ordering\n- VIN / ESN\n- Old part label\n- Dimensions\n- Application\n- Supplier catalog";
  return out;
}
async function rwdCleanAI(prompt, files){
  const q=String(prompt||"").trim();
  if(!q) return "";
  if(/^what\s+is\s+2\s*\+\s*2\??$/i.test(q) || /^2\s*\+\s*2$/.test(q)) return "4";
  const ctx={truck:state?.truck||{},settings:state?.settings||{},customer:state?.truck?.customer||"",quotes:(state?.quotes||[]).slice(0,3),invoices:(state?.invoices||[]).slice(0,3)};
  if(files && files.length) {
    try { return rwdCleanAnswer(await rwdCleanFn("rolling-wrench-vision-ai", {prompt:q,files,context:ctx})); } catch(e) { return "Photo received, but vision backend failed: " + e.message; }
  }
  if((/decode|what truck|vin/i.test(q)) && rwdCleanVIN(q)) {
    try { return rwdCleanAnswer(await rwdCleanFn("truck-search", {prompt:q,query:q,context:ctx})); } catch(e) { return rwdCleanLocalVin(q); }
  }
  if(/invoice|bill/i.test(q)) {
    try { return rwdCleanAnswer(await rwdCleanFn("build-invoice", {prompt:q,query:q,context:ctx})); } catch(e) { return rwdCleanFinanceText(q,"invoice"); }
  }
  if(/quote|estimate/i.test(q)) {
    try { return rwdCleanAnswer(await rwdCleanFn("build-quote", {prompt:q,query:q,context:ctx})); } catch(e) { return rwdCleanFinanceText(q,"quote"); }
  }
  if(rwdCleanLooksParts(q)) {
    const part_query=rwdCleanPartQuery(q);
    try { return rwdCleanFormatParts(await rwdCleanFn("oracle-parts-search", {part_query,query:part_query,prompt:part_query,raw_engine_text:q,context:ctx}), q); }
    catch(e) {
      try { return rwdCleanFormatParts(await rwdCleanFn("verify-part-web", {part_query,query:part_query,prompt:part_query,context:ctx}), q); }
      catch(e2) { return "Parts search unavailable: " + e2.message; }
    }
  }
  try { return rwdCleanAnswer(await rwdCleanFn("diesel-doc-ai", {prompt:q,question:q,query:q,context:ctx})); }
  catch(e) {
    try { return rwdCleanAnswer(await rwdCleanRender("/api/ai", {prompt:q,context:ctx})); }
    catch(e2) { return "RW AI backend unavailable. Finance, VIN, and parts fallback still work."; }
  }
}

/* Disable old AI controllers from winning */
window.rw92AskBackend = rwdCleanAI;
try { rw92AskBackend = rwdCleanAI; } catch(e) {}
try { v71aBrainAnswer = async function(text){ return await rwdCleanAI(text); }; } catch(e) {}
try { v87RunAI = function(text){ return rwdCleanAI(text); }; } catch(e) {}
try {
  rw8Run = async function(text){
    if(!text) return;
    if(String(text).trim().toLowerCase()==="clear"){ state.brainChats=[]; saveState(); if(typeof renderRW8Brain==="function")renderRW8Brain(); return; }
    if(typeof rw8Ensure==="function") rw8Ensure();
    if(typeof rw8Push==="function") rw8Push("user", text);
    const ans = await rwdCleanAI(text);
    if(typeof rw8Push==="function") rw8Push("ai", ans);
    if(typeof renderRW8Brain==="function") renderRW8Brain();
  };
} catch(e) {}
try {
